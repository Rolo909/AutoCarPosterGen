...
class PlaywrightAutoParser:
    ...
    def search_car(self, brand: str, model: str, year: Optional[int] = None) -> List[Dict]:
        ...
        try:
            ...
            print(f"📄 Открываю каталог марки: {brand_url}")
            
            self.page.goto(brand_url, wait_until='networkidle', timeout=30000)
            time.sleep(2)
            ...
            if '403' in html or 'forbidden' in html.lower():
                print("❌ Ошибка 403 - сайт заблокировал доступ")
                return []
            
            print("✅ Страница марки загружена")
            
            # Шаг 2: Ищем модель на странице
            soup = BeautifulSoup(html, 'html.parser')
            
            results = self._find_models_on_page(soup, brand, model, year)
            ...
            print("❌ Timeout - страница не загрузилась")
            return []
    ...
    def _find_models_on_page(self, soup: BeautifulSoup, brand: str, model: str, year: Optional[int]) -> List[Dict]:
        """Найти модели на странице каталога"""
        ...
        for link in links:
            ...
            # Проверяем модель в тексте ссылки или в URL
...
def main():
    ...
    # ПРИМЕР 1: BMW M5 F90 2019
    print("\n" + "="*80)
    print("ПРИМЕР 1: BMW M5 F90 2019")
    print("="*80)
    ...
    # ПРИМЕР 2: BMW 320i 2019
    print("\n" + "="*80)
    print("ПРИМЕР 2: BMW 320i 2019")
    print("="*80)
    ...
    # ПРИМЕР 3: Без года (найдет все годы)
    print("\n" + "="*80)
